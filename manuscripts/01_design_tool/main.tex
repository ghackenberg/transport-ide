...
For tool evaluation purposes, we currently provide five different implementations of this controller interface:
A \textbf{manual controller} (see Section~\ref{sec:controller-manual}), a \textbf{random controller} (see Section~\ref{sec:controller-random}), a \textbf{greedy controller} (see Section~\ref{sec:controller-greedy}), a \textbf{smart controller} (see Section~\ref{sec:controller-smart}), and a \textbf{switchable controller} (see Section~\ref{sec:controller-switch}) strategy.

\subsubsection{Manual controller}
\label{sec:controller-manual}

The manual controller strategy delegates routing, demand pickup, and charge decisions to the user of the tool using input dialogs.
...
Figure~\ref{fig:manual-controller-charge} shows the input dialog for charge decisions, which pops up when a vehicle arrives at the location of a charging station.
The dialog provides the name of the vehicle (\texttt{U} in the example), the location of the charging station (\texttt{A->B:50} in the example), and the two choice buttons (i.e.\ yes and no).
