...
The logic comprises four rules, which are processed in sequential order until one rule applies and makes a decision:
\begin{enumerate}
    \item When the battery of the vehicle is only half full or less, the strategy randomly selects an outgoing road segment with charging station if possible.
    ...
    \item Finaly, when none of the above rules applied, the strategy randomly selects any of the outgoing road segments of the current intersection.
\end{enumerate}
Moreover, the greedy control strategy always chooses to pick up demands or charge vehicle batteries if arriving at an unserved demand pick-up location or charging station.
